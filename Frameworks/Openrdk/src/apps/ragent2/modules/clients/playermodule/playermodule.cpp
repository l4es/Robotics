...
#define MODULE_NAME "PlayerModule"
...
#include "playermodule_names.h"

namespace RDK2 { namespace RAgent {

bool PlayerModule::initConfigurationProperties()
{
	SESSION_TRY_START(session)
		...
		session->createBool(PROPERTY_ENABLE_RANGER, "Enable ranger client", true);
#endif
...
#if ! defined PLAYER_VERSION_LT_2_1 && ! defined PLAYER_VERSION_LT_3
			if (session->getBool(PROPERTY_ENABLE_RANGER)) {
				rangerProxy = new RangerProxy(playerClient, 0);
				rangerProxy->RequestConfigure();
				RDK_DEBUG_PRINTF("Ranger enabled");
			}
#endif
...
#if ! defined PLAYER_VERSION_LT_2_1 && ! defined PLAYER_VERSION_LT_3
			if (session->getBool(PROPERTY_ENABLE_RANGER))        readRanger();
#endif
...
void PlayerModule::readGroundTruthPose() throw (SessionException)
{
...
void PlayerModule::readOdometry() throw (SessionException)
{
...
void PlayerModule::readLaser() throw(SessionException)
{
	...
	RPoint2od lpose = session->getPose(PROPERTY_ROBOT_LASER_POSE);
	rld->laserPose.x      = lpose.x;
	rld->laserPose.y      = lpose.y;
	rld->laserPose.theta  = lpose.theta;
...
#if ! defined PLAYER_VERSION_LT_2_1 && ! defined PLAYER_VERSION_LT_3
void PlayerModule::readRanger() throw(SessionException)
{
	if (!rangerProxy->IsValid()) return;

	RLaserData* rld = new RLaserData;
	int n = rangerProxy->GetRangeCount();

	rld->minReading = rangerProxy->GetMinRange();
	rld->maxReading = rangerProxy->GetMaxRange();
	for (int i = 0; i < n; i++){
		LaserData::LaserPoint lp;
		lp.reading = rangerProxy->GetRange(i);
		if (lp.reading <= rld->minReading)
		{
			// rangerProxy return 0 if the reading is not valid
			// (e.g. there is no obstacle => get maxReading)
			lp.reading = rld->maxReading;
		}
		//lp.intensity  = rangerProxy->GetIntensity(i);
		rld->points.push_back(lp);
	}
	...
	rld->timestamp.setToNow();
	rld->ipc_timestamp.setToNow();
	player_pose3d_t lpose = rangerProxy->GetDevicePose();
	rld->laserPose.x      = lpose.px;
	rld->laserPose.y      = lpose.py;
	rld->laserPose.z      = lpose.pz;
	rld->laserPose.theta  = lpose.pyaw;
	rld->laserPose.phi    = lpose.ppitch;
	rld->laserPose.gamma  = lpose.proll;
	rld->minTheta = rangerProxy->GetMinAngle();
	rld->maxTheta = rangerProxy->GetMaxAngle();
	rld->updateThetas();

	Point2od globalRobotPose;
	if (session->getBool(PROPERTY_USE_GROUND_TRUTH_FOR_LASER) && session->getBool(PROPERTY_ENABLE_GROUND_TRUTH))
	{
		char * cstr;
		string name = session->getString(PROPERTY_ROBOT_NAME);
		cstr = new char [name.size()+1];
...
void PlayerModule::readSonar() throw(SessionException)
{
...
void PlayerModule::readImage() throw ( SessionException )
{
...
void PlayerModule::readFiducial() throw ( SessionException )
{
...
void PlayerModule::updatePlayerSpeed(Session* session) throw (SessionException)
{
...
bool PlayerModule::parseRobotCmd(cstr cmd, Session* session)
{
	bool r = false;
...
void PlayerModule::asyncAgentCmd(cstr cmd)
{
