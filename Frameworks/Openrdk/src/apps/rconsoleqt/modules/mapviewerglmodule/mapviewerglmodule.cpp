...
bool MapViewerGlModule::init()
{
	...
	if (session->getString(PROPERTY_URL) != "") {
		...
		v->push_back(new RString(prefix + "/robot/odometryPose|F|100,100,100|" + prefix + "/robot/"));
		v->push_back(new RString(prefix + "/robot/estimatedPose|F|255,0,0|" + prefix + "/robot/"));
		v->push_back(new RString(prefix + "/robot/targetPose|F|0,255,0|" + prefix + "/robot/"));
...
void MapViewerGlModule::refreshViewer()
{
	...
	SESSION_TRY_START(guiSession)
		...
		for (size_t i = 0; i < v->size(); i++) {
			if (v->getItem(i)->value[0] != '#')
				viewer->addPose(v->getItem(i)->value);
			else
				viewer->clearPose(v->getItem(i)->value);
		}
...
void MapViewerGlModule::saveOptionsInConfig()
{
	...
	for (QListViewItemIterator it(lboxShownMaps); it.current(); ++it) {
		...
		if (itm) {
			mapUrls->push_back(new RString(QString("%1|%2")
				.arg(QString(itm->state() == QCheckListItem::Off ? "#" : "") + it.current()->text(0))
	...
	for (QListViewItemIterator it(lboxShownPoses); it.current(); ++it) {
		RCheckListItem* itm = dynamic_cast<RCheckListItem*>(it.current());
		bool poseTrails = false;
		if (itm) {
			poseTrails = itm->state() == QCheckListItem::On;
			poseUrls->push_back(new RString(QString("%1|%2|%3|%4")
				.arg(QString(itm->state() == QCheckListItem::Off ? "#" : "") + itm->text(0)) // posetrails forces checked to 1
				.arg(QString(poseTrails ? "T" : "F"))
				.arg(itm->text(2))
				.arg(itm->text(3)).latin1()));
		}
		else
		{
			RDK_ERROR_STREAM("Not an RCheckListItem");
		}
	}
	guiSession->unlock(PROPERTY_POSE_URLS);
...
void MapViewerGlModule::updateSidePanelWithConfig()
{
	...
	for (size_t i = 0; i < shownPoses.size(); i++) {
		...
		if (v.size() != 4) {
			...
			bool selected = true;
			if (v[0].size() > 1 && v[0][0] == '#') { selected = false; v[0] = v[0].substr(1);}
			RCheckListItem* itm = new RCheckListItem(lboxShownPoses, v[0], QCheckListItem::CheckBox);
			itm->setTristate(true);
			// the transition among state is:
			// Off -> NoChange -> On
			// We use the second state (NoChange) to enable pose drawing
			// and the third (On) to enable pose trails
			itm->setState(selected ? QCheckListItem::NoChange : QCheckListItem::Off );
			int index = 1;
			if (v[index].size() == 1)
			{
				if(v[index][0] == 'T')
				{
					itm->setState(QCheckListItem::On);
				}
			}
			else
			{
			...
			vector<string> c = TextUtils::tokenize(v[index+1], ",");
			if (c.size() != 3) {
				RDK_ERROR_PRINTF("Malformed pose information (%s) %s", shownPoses[i].c_str(), v[index+1].c_str());
			}
			else {
				QPixmap pm(36, 12);
			...
			itm->setText(2, v[index+1]);
			itm->setText(3, v[index+2]);
