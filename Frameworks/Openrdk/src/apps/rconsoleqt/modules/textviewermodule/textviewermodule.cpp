...
#include "textviewermodule.h"

#include <qlabel.h>

#include <rdkcore/logging/logging.h>
#define LOGGING_MODULE "TextViewerModule"

#include <rdkcore/rprimitive/rstring.h>

namespace RDK2 { namespace RConsoleQt {
	
QWidget* TextViewerModule::createWidget()
{
	string myUrl = "<error>";
	...
	editLayout->addWidget(textEdit = new QTextEdit(widget));
	textEdit->setReadOnly(true);
	
	editWidgets.push_back(textEdit);
	
	return widget;
}

RDK2::Object* TextViewerModule::buildRObject()
{
	RDK2::Object* obj = new RString();
	if (obj->loadFromStringRepresentation(textEdit->text().latin1())) {
		return obj;
	}
	else {
		delete obj;
...
void TextViewerModule::refreshWidgetFields()
{
	QT_THREAD_GUARD()
	...
	textEdit->clear();
	textEdit->setText(QString(value));
}
	
MODULE_FACTORY(TextViewerModule);
