...
#define LOGGING_MODULE "MapViewerGlModule(GUI)"
...
#include "../../rqcommon/rchecklistitem.h"

#include <cfloat>

namespace RDK2 { namespace RConsoleQt {

void MapViewerGlModule::slot_sliderImageScale_valueChanged(int v)
{
	QT_THREAD_GUARD()
...
void MapViewerGlModule::slot_sliderMapAlpha_valueChanged(int v)
{
	QT_THREAD_GUARD()
...
void MapViewerGlModule::slot_timerSaveToConfig_timeout()
{
	saveOptionsInConfig();
}

void MapViewerGlModule::slot_cboRefreshRate_activated(int idx)
{
	QT_THREAD_GUARD()
	switch (idx) {
...
void MapViewerGlModule::slot_btnChangeColorPoseSelected_clicked()
{
	QT_THREAD_GUARD()
...
void MapViewerGlModule::slot_btnRemoveShownPoseSelected_clicked()
{
	QT_THREAD_GUARD()
	QListViewItem* itm = lboxShownPoses->currentItem();
	if (!itm) return;
	string url = itm->text(0).latin1();
	...
	int i=0;
	for (Vector<RString>::iterator it=poseUrls->begin();
			 it!=poseUrls->end();
			 ++it, ++i)
	{
		if(poseUrls->getItem(i)->value == url)
		{
			poseUrls->erase(it,true);
			break;
		}
	}
	guiSession->unlock(PROPERTY_POSE_URLS);
	SESSION_END_CATCH_TERMINATE(guiSession)
	lboxShownPoses->takeItem(itm);
}

void MapViewerGlModule::slot_lboxShownMaps_selectionChanged(QListViewItem* itm)
{
	QT_THREAD_GUARD()
	if (itm && itm->isSelected()) {
		...
		RMapImage* rmi = guiSession->getObjectAsL<RMapImage>(url);
		lblShownMapInfo->setText(QString("<b>Map information:</b><br/>"
									"World position:<br/>  x = %1 m; y = %2 m; theta = %3&deg;<br/>"
									"World size: %4 x %5 meters<br/>"
									"Image size: %6 x %7 pixels")
									.arg(rmi->x).arg(rmi->y).arg(rad2deg(rmi->theta), 0, 'g', 2)
									.arg(rmi->getRealWidth()).arg(rmi->getRealHeight())
									.arg(rmi->image->getWidth()).arg(rmi->image->getHeight()));
...
void MapViewerGlModule::slot_lboxShownPoses_selectionChanged(QListViewItem* itm)
{
	QT_THREAD_GUARD()
	if (itm && itm->isSelected()) {
		Session* guiSession = RqCommon::getGuiSession(getModuleName());
...
QWidget* MapViewerGlModule::createSideWidget(QWidget* parent)
{
	QT_THREAD_GUARD(0)
	...
	
	{
		...
		vl->addWidget(new QLabel("<b>How to move, rotate, etc.:</b><br/>"
			...
			gboxImageScale = new QGroupBox(1, Qt::Vertical, tab);
				...
				sliderImageScale = new QSlider(1, 1000, 10, 100, Qt::Horizontal, gboxImageScale);
		...
		vl->addWidget(gboxImageScale);
			...
			QGroupBox* gbox = new QGroupBox(2, Qt::Vertical, tab);
				...
				chkCloneMap = new QCheckBox("Clone map instead of lock it", gbox);
		vl->addWidget(gbox);
		...
		QVBoxLayout* vl = new QVBoxLayout(tab);
			...
			QTabWidget* tabwp = new QTabWidget(tab);
				...
				QVBoxLayout* vll = new QVBoxLayout(tabp);
					...
					lboxShownPoses->installEventFilter(this);
					lboxShownPoses->setAcceptDrops(true); // this does not work, yet
					...
					lboxShownPoses->setColumnWidthMode(2,QListView::Manual);
					lboxShownPoses->setColumnWidth(2,0);
					lboxShownPoses->header()->setResizeEnabled(false,2);
					lboxShownPoses->setColumnWidthMode(3,QListView::Manual);
					lboxShownPoses->setColumnWidth(3,0);
					lboxShownPoses->header()->setResizeEnabled(false,3);
				vll->addWidget(gbox);
				vll->addWidget(new QLabel("<b>How to show trails:</b><br/>"
							"Double-select the url of which you want to display trails. "
							"The text is red if the selected url "
							"has trails enabled.", tabp));
					...
					QObject::connect(btnRemoveShownPoseSelected, SIGNAL(clicked()), this, SLOT(slot_btnRemoveShownPoseSelected_clicked()));
...
bool MapViewerGlModule::eventFilter(QObject *o, QEvent *e)
{
			...
			if (event) {
				...
				if (QTextDrag::decode(event, qsUrl)) {
					...
					if (guiSession->getPropertyClassName(u) == "RItemOnMapVector") {
						RCheckListItem* itm = new RCheckListItem(lboxShownProps, u, QCheckListItem::CheckBox);
						itm->setOn(true);
					}
					SESSION_END_CATCH_TERMINATE(guiSession)
					return true;
	...
	else if (o == lboxShownPoses) {
		if (e->type() == QEvent::Drop) {
			Session* guiSession = RqCommon::getGuiSession(getModuleName());
			SESSION_TRY_START(guiSession)
			RDK_INFO_STREAM("Event=" << e->type());
			QDropEvent* event = dynamic_cast<QDropEvent*>(e);
			if (event) {
				QString qsUrl;
				if (QTextDrag::decode(event, qsUrl)) {
					Url u = qsUrl.latin1();
					RDK_INFO_STREAM("Event=" << guiSession->getPropertyClassName(u));
					if (guiSession->getPropertyClassName(u) == "RPoint2od") {
...
QWidget* MapViewerGlModule::createViewerWidget(QWidget* parent)
{
	QT_THREAD_GUARD(0)
...
void MapViewerGlModule::slot_viewerWidget_mousePressed(double x, double y, Qt::ButtonState bs)
{
...
void MapViewerGlModule::slot_viewerWidget_mouseMoved(double x, double y, Qt::ButtonState bs)
{
	QT_THREAD_GUARD()
	if (!(bs & Qt::LeftButton)) {
		lblStatus->setText(QString("(%1, %2)").arg(x, 0, 'f', 2).arg(y, 0, 'f', 2));
	}
	else {
		if (posing) {
...
void MapViewerGlModule::slot_viewerWidget_mouseReleased(double x, double y, Qt::ButtonState /*bs*/)
{
	QT_THREAD_GUARD()
	if (posing) {
		if (lboxEditablePoses->currentItem() != -1) {
...
void MapViewerGlModule::slot_viewerWidget_scaleChanged(double s)
{
	QT_THREAD_GUARD()
