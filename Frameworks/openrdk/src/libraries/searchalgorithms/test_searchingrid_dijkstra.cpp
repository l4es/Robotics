...
class DijkstraWithGrid : public SearchAlgorithms::Dijkstra<Point> {
public:
	int w, h;
...
public:
	DijkstraWithGrid(int w, int h) : SearchAlgorithms::Dijkstra<Point>(), w(w), h(h)
	{
		grid = new bool*[w];
...
vector<Point> DijkstraWithGrid::getSuccessorsOf(const Point& p)
{
	vector<Point> r;
...
double DijkstraWithGrid::computeCost(const Point& state1, const Point& state2)
{
...
{
	//int grid[20][20];
	DijkstraWithGrid dijkstra(40, 40);
